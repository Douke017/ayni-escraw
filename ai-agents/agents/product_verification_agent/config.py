...
class ProductVerificationConfig(BaseModel):
    ...
    gemini_model: str = Field(default="gemini-2.0-flash", description="Gemini LLM model identifier")
    ...
    mock_llm_mode: bool = Field(
        default_factory=lambda: os.getenv("MOCK_LLM_MODE", "true").lower() in ("true", "1", "yes"),
